...
int main() {
    ...
    while (nPar--) {
        ...
        for (int i = 0; i < 3; ++i) {
            cin >> cur[i].fi >> cur[i].se; --cur[i].fi;
            for (int j = 0; j < i; ++j) addEdge(getId(cur[i].fi, cur[i].se), getId(cur[j].fi, cur[j].se) );
        }
    }
    ...
    for (int u = 0; u < sz(gr); ++u) if (!disc[u]) dfs(u);

    for (int u = 0; u < n; ++u) if (inSCC[getId(u, 'B')] == inSCC[getId(u, 'R')]) return cout << "-1\n", 0;

    vector<char> ans(n);
    for (int u = 0; u < n; ++u) ans[u] = inSCC[getId(u, 'B')] < inSCC[getId(u, 'R')] ? 'B' : 'R';
